...
document.addEventListener('DOMContentLoaded', async () => {
    ...
    if (!token) {
        errorDiv.textContent = 'You must be logged in to view your profile.';
    ...
    try {
        ...
        if (data.ReturnCode == 200) {
            ...
            profileImage.src = cartoonUrls[user.profile_img] || `https://cdn.pixabay.com/photo/2015/10/05/22/37/blank-profile-picture-973460_1280.png`;
            userName.textContent = user.name || 'Unknown';
            userPhone.textContent = `${user.country_code} ${user.number}` || 'N/A';
            userAddress.textContent = user.address || 'N/A';
            userPostcode.textContent = user.postcode || 'N/A';
            userEmail.textContent = user.email || 'N/A';

            // Display interested books
            if (user.interestedBooks && user.interestedBooks.length > 0) {
                interestedBooksDiv.innerHTML = user.interestedBooks.map(book => `<span class="badge bg-primary me-1 mb-1">${book}</span>`).join('');
            } else {
                interestedBooksDiv.textContent = 'No interested books listed.';
            }

            errorDiv.style.display = 'none';
        } else {
            errorDiv.textContent = data.message || 'Failed to load profile.';
            errorDiv.style.display = 'block';
        }
    } catch (err) {
